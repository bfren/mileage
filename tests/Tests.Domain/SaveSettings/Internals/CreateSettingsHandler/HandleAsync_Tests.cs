...
{
	private sealed class Setup : Setup<ISettingsRepository, SettingsEntity, SettingsId, CreateSettingsHandler>
	{
	...
	[Fact]
	public async Task Calls_Repo_CreateAsync__Receives_None__Returns_Result()
	{
		// Arrange
		var (handler, v) = GetVars();
		var command = new CreateSettingsCommand(new(), new());
