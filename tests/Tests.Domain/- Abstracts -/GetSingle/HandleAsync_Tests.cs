...
public abstract class HandleAsync_Tests
{
	...
	public abstract Task Test04_Calls_FluentQuery_QuerySingleAsync__Returns_Result();
		...
		where THandler : QueryHandler<TQuery, TModel>
	{
		...
		internal async Task Test04(Func<THandler, TQuery, Task<Maybe<TModel>>> handle)
		{
			// Arrange
			var (handler, v) = GetVars();
			var model = NewModel;
