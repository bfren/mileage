...
public sealed class HandleAsync_Tests
{
	private Vars Setup()
	{
		...
		var query = new GetAnnualMileageReportDataQuery(IdGen.LongId<AuthUserId>(), Rnd.DateTime, Rnd.DateTime);
	...
	public async Task Calls_Db_QueryAsync__With_Correct_Values()
	{
		...
		// Assert
		await v.Db.Received().QueryAsync<AnnualMileageReportJourney>(sql, v.Query);
	}
	...
	public async Task Calls_Db_QueryAsync__Returns_Result()
	{
		...
		v.Db.QueryAsync<AnnualMileageReportJourney>(query: default!, default)
