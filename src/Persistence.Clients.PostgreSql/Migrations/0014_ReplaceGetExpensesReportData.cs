...
/// <summary>
/// Migration 0014: Replace function to get expenses report data
/// </summary>
[Migration(14, "Replace function to get expenses report data")]
public sealed class ReplaceGetExpensesReportData : Migration
{
	...
	protected override void Up() => Execute($@"
		...
		SELECT 
			...
			(
				...
				WHERE {J(x => x.ToPlaceIds)} @> jsonb_build_array({T(x => x.Id)})
				OR {J(x => x.ToPlaceIds)} @> jsonb_build_array({T(x => x.Id)}::text)
			) AS ""{A.Col.To}""
		FROM
